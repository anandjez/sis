...
from stint_sampler.targets.targets import gmm
import time
...
def langevin(Nsamples,Nt,alpha=1.0,delt=1e-2,inner_Nt = 2):
...
d=2
target,sampler_gt = gmm()
target_score = vmap(grad(lambda x:target(x[None,:])[0]))

sampler_model = genSampler(modelPath,outPath)
...
for i in range(len(Nt)):
    ...
    samples_langevin = langevin(Ns,Nt[i],alpha=10.0)
